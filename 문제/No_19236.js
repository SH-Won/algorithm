// const input = [
// '7 6 2 3 15 6 9 8',
// '3 1 1 8 14 7 10 1',
// '6 1 13 6 4 3 11 4',
// '16 1 8 7 5 2 12 2'
// ]
// const input = [
// '16 7 1 4 4 3 12 8',
// '14 7 7 6 3 4 10 2',
// '5 2 15 2 8 3 6 4',
// '11 8 2 4 13 5 9 4',
// ]
// const input = [
// '12 6 14 5 4 5 6 7',
// '15 1 11 7 3 7 7 5',
// '10 3 8 3 16 6 1 1',
// '5 8 2 7 13 6 9 2',
// ]
const input = [
'2 6 10 8 6 7 9 4',
'1 7 16 6 4 2 5 8',
'3 7 8 6 7 6 14 8',
'12 7 15 4 11 3 13 3',
]
//const fs = require('fs');
//const input = fs.readFileSync('/dev/stdin').toString().trim().split('\n');
class Fish{
    constructor(y,x,dir,number){
        this.y = y;
        this.x = x;
        this.dir = dir;
        this.number = number;
    }
}
const solution = () =>{
    const dy = [0,-1,-1,0,1,1,1,0,-1];
    const dx = [0,0,-1,-1,-1,0,1,1,1];
    const isValidPos = (y,x) => (y>=0 && x>=0 && y<4 && x<4);
    let map = Array.from({length:4},()=>Array(4));
    let fishes = [];
    let shark = {y:null,x:null,dir:null,sum:0};
    for(let y=0; y<4; y++){
        const fishInfo = input[y].split(' ').map(Number);
        let i=0;
        let x=0;
        while(i < fishInfo.length){
            const [num,dir] = [fishInfo[i],fishInfo[i+1]];
            if(y===0 && x ===0) {
              shark.y = y , shark.x = x , shark.dir = dir , shark.sum =num;
              i+=2;
              x++;
              map[y][x] = 0;
              continue;
            }
            fishes.push(new Fish(y,x,dir,num));
            i+=2;
            x++;
        }
    }
    fishes.sort((a,b)=>a.number - b.number);
    fishes.forEach(fish => map[fish.y][fish.x] = fish);
    // fishes.unshift(new Fish(null,null,null,0));
    
    const fishMove = (fishes,map,shark) =>{
        for(let i=0; i<fishes.length; i++){
            let fish = fishes[i];
            let {y,x,dir} = fish;
            let [ny,nx] = [y+dy[dir],x+dx[dir]];
            while(true){
                if(!isValidPos(ny,nx) || (ny===shark.y && nx ===shark.x)){
                   dir = dir+1 > 8 ? 1 : dir+1;
                   [ny,nx] = [y+dy[dir],x+dx[dir]];
                   continue;
                }
                else break;
            }
            fish.dir = dir;
            if(map[ny][nx]){
                let swapFish = map[ny][nx];
                fish.y = ny , fish.x = nx , swapFish.y = y , swapFish.x = x;
                [map[y][x],map[ny][nx]] = [map[ny][nx],map[y][x]]
            }
            else{
                fish.y = ny, fish.x = nx;
                map[ny][nx] = fish;
                map[y][x] = 0;
            }
            
        }
    }
    
    let max = 0;
    const dfs = (fishes,map,shark) =>{
        if(shark.sum > max){
            max = shark.sum;
        }
        fishMove(fishes,map,shark);
        // fishes, map 이 다 바뀜 스코프

        for(let i=1; i<4; i++){
            const {y,x,dir} = shark;
            let [ny,nx] = [y+dy[dir]*i,x+dx[dir]*i];
            if(!isValidPos(ny,nx) || !map[ny][nx]) continue;
            // copyFishes copyMap 을 만들어 스코프를 일치시킴
            let copyFishes = Array.from({length:fishes.length},(_,i)=> new Fish(fishes[i].y,fishes[i].x,fishes[i].dir,fishes[i].number));
            let copyMap = Array.from({length:4},() => Array(4).fill(0));
            copyFishes.forEach(fish => copyMap[fish.y][fish.x] = fish);
            const fish = copyMap[ny][nx];
            const fishIndex = copyFishes.findIndex(el => el ===fish);
            const newShark = {y : fish.y ,x :fish.x , dir: fish.dir , sum:shark.sum+fish.number};
            copyFishes.splice(fishIndex,1);
            copyMap[ny][nx] = 0;
            dfs(copyFishes,copyMap,newShark);

        }
    }
    dfs(fishes,map,shark);
    console.log(max);

}
solution();

