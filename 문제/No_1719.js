const input = ['6 10','1 2 2','1 3 1','2 4 5','2 5 3','2 6 7','3 4 4','3 5 6','3 6 7','4 6 4','5 6 2']
// const fs = require('fs');
...
const dijkstra = (start,edge,table) =>{
    const dist = Array(table.length).fill(Infinity);
    const path = Array(table.length);
    dist[start] = 0;
...
const solution = input =>{
    ...
    for(let i=1; i<=n; i++){
        dijkstra(i,edge,table[i]);
    }
    let answer = '';
    for(let i=1; i<table.length; i++){
        answer += `${table[i].slice(1).join(' ')}\n`;
    }
