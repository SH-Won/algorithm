...
// 자바스크립트에서 원시타입은 그 값을 그대로 복사하지만,
// String, Number, null , boolean 값 , undefined , Symbol(ES6) 를 제외한
// 모든 객체는 참조형태이므로 항상 이 개념을 숙지해서 코드를 적어야한다.

// 정리
// 자바스크립트의 원시타입을 제외하면 모든 것은 객체이다.
// 자바스크립트의 String Number boolean 은 원시타입이면서 래퍼객체이다.
// 원시타입을 다른 변수에 할당할때 그값을 그대로 복사하지만,
// 객체를 다른변수에 할당할때는 그값을 복사하는것이 아니라 실제 객체를 바라보고 있는 주소를 복사한다.
